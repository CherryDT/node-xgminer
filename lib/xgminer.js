...
xgminer.prototype.send = function (command, parameter) {
  ...
  try {
    ...
    socket.setTimeout(self.options.timeout || 5000, function () {
      deferred.reject(new Error('Socket timed out'));
    });
    ...
    socket.connect({
      ...
      socket.on('end', function () {
        ...
        // We also replace }{ with },{ because AntMiners are sending invalid JSON in the "stats" call
        // They send stuff like [{.....}{.....}] with missing commas...
  ...
  // Return the promise immediately (but destroy socket once we are done)
  return deferred.promise.finally(function () { socket.destroy(); });
}
